...
init_repo <- function(name, org = "openscapes", path = getwd(),
  ...
  page <- 1
  rn_growing <- TRUE
  repo_names <- gh("GET /orgs/{org}/repos", org = org, per_page = 100, page = page) %>% 
    map_chr(~.x$"name")
  ...
  repo_initialized <- tryCatch(gert::git_find(repo_path), error = function(e) FALSE)
